...
/* 
 * this thread, in the main proc, allows fsysproc 
 * to get a window made without doing graphics 
 */
...
iconinit(void)
{
	...
	unsigned int tag_colors[4] = {
		c_tag_bg, c_tag_fg, c_tag_hl_bg, c_tag_hl_fg
	};
	unsigned int txt_colors[4] = {
		c_txt_bg, c_txt_fg, c_txt_hl_bg, c_txt_hl_fg
	};
	unsigned int ui_colors[4] = {
		c_column_button, c_scrollbar_bg, c_tmp_button, c_window_button
	};
	unsigned int mhl_colors[2] = {
		c_button2_hl, c_button3_hl
	};
...
/*
 * /dev/snarf updates when the file is closed, so we must open
 * our own fd here rather than use snarffd
 *
...
ismtpt(char *file)
{
	...
	n = strlen(mtpt);
	return strncmp(file, mtpt, n) == 0 && ((n > 0 && mtpt[n-1] == '/')
	 || file[n] == '/' || file[n] == 0);
}
